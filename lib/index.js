...
var populate = async function populate() {
  if (getTimingEntries() === null || currentOptions.isAutoReset) {
    currentOptions.configInterface.get(currentOptions.configUrl).then(function (response) {
      return currentOptions.storageManagerInterface.setItem(currentOptions.key, response.data[currentOptions.key]);
    }).catch(function (err) {
